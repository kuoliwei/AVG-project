...
public class PortraitKeyDropdownDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        // === 修改區：改為從 speaker.portraitGroup 讀取對應的 Addressables Group ===
        var settings = AddressableAssetSettingsDefaultObject.Settings;
        List<string> keys = new List<string> { "none" }; // 預設第一個為 none

        if (settings != null && speaker != null && !string.IsNullOrEmpty(speaker.portraitGroup))
        {
            var group = settings.groups.FirstOrDefault(g => g != null && g.Name == speaker.portraitGroup);
            if (group != null)
            {
                // 將該 Group 中的所有 Addressables key 加入選單
                keys.AddRange(group.entries.Select(e => e.address));
            }
        }
        // === 修改區結束 ===
